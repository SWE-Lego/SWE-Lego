...
def fix_tool_call_arguments(response: ModelResponse) -> ModelResponse:
    """
    Postprocess model response to fix malformed tool call arguments.

    Specifically handles cases where Qwen3-Coder-480B generates quoted lists
    for the view_range argument (e.g., "view_range": "[1, 50]" instead of [1, 50]).

    Args:
        response: The ModelResponse from the LLM

    Returns:
        The ModelResponse with fixed tool call arguments
    """
    # Only apply fix if environment variable is set to true
    if os.getenv('ENFORCE_INT_VIEW_RANGE', 'false').lower() != 'true':
        return response
    ...
    for tool_call in response.choices[0].message.tool_calls:
        ...
        try:
            ...
            # Specifically target the view_range argument
            if 'view_range' in args_dict and isinstance(args_dict['view_range'], str):
                view_range = args_dict['view_range'].strip()

                # Check if it's a quoted list/array like "[1, 50]"
                if view_range.startswith('[') and view_range.endswith(']'):
                    try:
                        # Try to parse as JSON array
                        parsed_view_range = json.loads(view_range)
                        if isinstance(parsed_view_range, list):
                            args_dict['view_range'] = parsed_view_range
                            tool_call.function.arguments = json.dumps(
                                args_dict, ensure_ascii=False
                            )
                            logger.debug(
                                f'Fixed quoted view_range argument: {view_range} -> {parsed_view_range}'
                            )
                    except json.JSONDecodeError:
                        # If parsing fails, leave as-is
                        logger.debug(
                            f'Could not parse view_range as JSON array: {view_range}'
                        )
                        pass
            ...
            logger.debug(
                f'Could not parse tool call arguments, leaving as-is: {tool_call.function.arguments}'
            )
            continue
    ...
    def __init__(
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if mock_function_calling and 'tools' in kwargs:
                # check env variable to decide whether to add in-context learning example
                add_in_context_learning_example = os.getenv('ADD_IN_CONTEXT_LEARNING_EXAMPLE', 'true').lower() == 'true'
            ...
            with warnings.catch_warnings():
                ...
                # Postprocess the response to fix tool call argument formatting issues
                # Specifically handles Qwen3-Coder models that quote view_range list arguments
                resp = fix_tool_call_arguments(resp)
